# Pre-Launch Checklist

## ✅ Completed

- [x] Merge gemini.html into index.html
- [x] Extract CSS to styles.css
- [x] Delete gemini.html
- [x] Select best venue images (5 selected from 27 reviewed)
- [x] Copy images to project folder
- [x] Update HTML to load images automatically
- [x] Configure .gitignore for GitHub
- [x] Create documentation (README, DEPLOYMENT, SUMMARY)
- [x] Verify responsive design
- [x] Test countdown timer functionality (includes seconds)
- [x] Verify drag-and-drop still works
- [x] Implement dynamic date formatting with BST timezone
- [x] Integrate Google Sheets RSVP backend
- [x] Add music request field and consent checkbox to RSVP
- [x] Add Geordie dialect validation messages
- [x] Configure custom domain (www.86groove.space)
- [x] Update venue image to bar interior
- [x] Enhanced RSVP error handling
- [x] Implement WebP image optimization (93% size reduction)
- [x] Add picture elements with fallback support
- [x] Create Node.js optimization script

## 🔲 Before First Deployment

- [ ] Review and test site locally (open index.html in browser)
- [ ] Verify all images display correctly
- [ ] Check mobile responsiveness
- [ ] Decide on image optimization (run optimize-images.sh or keep originals)
- [ ] Commit all files: `git add . && git commit -m "Add party website"`
- [ ] Push to GitHub: `git push origin main`
- [ ] Enable GitHub Pages in repository settings

## 🔲 Before Sharing with Guests

- [ ] Add individual host photos (currently placeholders)
- [ ] Update host names in "The Ringleaders" section
  - Line 58: Change "The Birthday Boy" to actual name
  - Line 65: Change "The Other One" to actual name
- [ ] Verify event details are correct:
  - [ ] Date: Sunday May 24, 2026
  - [ ] Time: 17:00 (5 PM)
  - [ ] Venue: The Grove, Newcastle upon Tyne
- [x] Test RSVP form (Google Sheets integration active)
- [ ] Consider privacy for faces in venue photos

## 🔲 Optional Enhancements

- [x] Optimize images (WebP: 1.3MB, originals: 18MB for fallback)
- [x] Add functional RSVP backend (complete with consent)
- [ ] Add Google Analytics tracking
- [ ] Create custom domain (instead of github.io)
- [ ] Add social media meta tags for sharing
- [ ] Create favicon
- [ ] Add "Add to Calendar" button
- [ ] Include venue map/directions
- [ ] Add accommodation suggestions for out-of-town guests

## 🎯 Current Status

**Deployed**: YES ✅ (www.86groove.space)
**Optimized**: YES ✅ (WebP images, 93% size reduction)
**Ready to Share**: NO ⚠️ (needs host photos and names)
**RSVP Functional**: YES ✅ (Google Sheets + consent checkbox)

**Estimated time to deployment**: 5 minutes
**Estimated time to guest-ready**: 30 minutes (with host photos)

## 📝 Notes

- Website is fully functional without host photos
- Drag-and-drop works for quick updates
- Can deploy now and add host photos later
- All venue images are high-quality originals from The Grove

